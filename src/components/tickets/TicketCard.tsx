...
export function TicketCard({ ticket, className }: { ticket: IncidentWithNames; className?: string }) {
  return (
    <Card
      className={cn(
        "border-border bg-surface shadow-[0_18px_30px_-24px_rgba(15,23,42,0.35)]",
        className
      )}
    >
      <CardContent className="space-y-4 p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs font-semibold text-muted">{formatIncidentCode(ticket.incidentId)}</p>
